...
async function fetchReservations() {
  ...
  try{
    let b=await fetch(`${config.backendEndpoint}/reservations/`);
  let d=await b.json();
  return d;
  }catch(err){
    return null;
...
function addReservationToTable(reservations) {
  ...
  */
  for(let i=0;i<reservations.length;i++){
    ...
    var m=t.toLocaleString(undefined,{month:"long"});
    var day=t.getDate();
    var y=t.getFullYear();
    var booktime=t.toLocaleString("en-IN").split(" ");
    ...
    tr.innerHTML=`
       ...
       <td>${day} ${m} ${y}, ${booktime[1]} ${booktime[2]}</td>
